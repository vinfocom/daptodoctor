...
import {
    getDoctorUnreadPatientIdsSnapshot,
    markDoctorPatientChatRead,
    markDoctorPatientChatUnread,
    subscribeDoctorChatState,
...
const Patients = () => {
    ...
    const [unreadPatientIds, setUnreadPatientIds] = useState<Set<number>>(
        () => new Set(getDoctorUnreadPatientIdsSnapshot())
    );
    ...
    useEffect(() => {
        return subscribeDoctorChatState(() => {
            setUnreadPatientIds(new Set(getDoctorUnreadPatientIdsSnapshot()));
        });
    }, []);
    ...
    const checkNotifications = useCallback(async () => {
        ...
        try {
            ...
            if (result?.latestMessage?.patientId && !result.latestMessage.isAnnouncement) {
                markDoctorPatientChatUnread(result.latestMessage.patientId);
            }
        } catch {
            // ignore
        }
    ...
    useEffect(() => {
        ...
        socket.on('receive_message', (msg: any) => {
            ...
            if (!pid) return;
            markDoctorPatientChatUnread(pid);
        });
