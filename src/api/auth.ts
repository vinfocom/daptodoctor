...
async function saveDoctorPushTokenWithRetry(pushToken: string, authToken?: string) {
  const maxAttempts = 3;

  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      if (__DEV__) {
        console.log(`[push] doctor push token save attempt ${attempt}/${maxAttempts}`);
      }
...
export const saveDoctorPushToken = async (pushToken: string, authToken?: string) => {
  await saveDoctorPushTokenWithRetry(pushToken, authToken);
};
